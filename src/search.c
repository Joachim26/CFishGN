              ...
              && pos->rootDepth > Limits.depth))
  {
    ...
    for (int pvIdx = 0; pvIdx < multiPV && !Threads.stop; pvIdx++) {
      ...
      if (pos->rootDepth >= 4) {
        ...
        delta = 17 + abs((int)previousScore) / 64;   //Orig 16384
    ...
    Depth depth, bool cutNode, const int NT)
{
  ...
  if (  !PvNode
      && ss->ttHit
      && tte_depth(tte) > depth - (pos->threadIdx % 2 == 1)
  ...
  while ((move = next_move(pos, moveCountPruning))) {
        ...
        && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
    {
          ...
          || givesCheck)
      {
        ...
        if (   !is_empty(to_sq(move))
            ...
            && lmrDepth < 6
            && !inCheck
            && ss->staticEval + 342 + 238 * lmrDepth + PieceValue[EG][piece_on(to_sq(move))]
             + (*pos->captureHistory)[movedPiece][to_sq(move)][type_of_p(piece_on(to_sq(move)))] / 8 < alpha)
            continue;
        ...
        lmrDepth = max(0, lmrDepth - (beta - alpha < pos->rootDelta)); //orig / 4
        ...
        &&  tte_depth(tte) >= depth - 3)
    {
      ...
      if (value < singularBeta) {
        ...
        if (   !PvNode
            && value < singularBeta - 90  //Orig - 93
            && ss->doubleExtensions < 4)  //Orig < 3
        {
          extension = 2;
        }
      }
    ...
    // Quiet ttMove extensions
             ...
             && move == ttMove
             && move == ss->killers[0]
             && (*cmh)[movedPiece][to_sq(move)] >= 10000)
    {
      extension = 1;
    }
            ...
            || (cutNode && (ss-1)->moveCount > 1)))
    {
      ...
      // Increase reduction at non-PV nodes when the best move doesn't change frequently
